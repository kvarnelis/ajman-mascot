Create Codex pet row `running-right` for `ajman`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Ajman exactly as shown in the canonical reference: tuxedo pattern, yellow-green eyes, white paws, white chest bib, face and proportions, with the visibly notched rescue ear-tip on the screen-right ear in the frontal reference; the notch is mandatory in every frame. Preserve silhouette, face, palette, material, proportions, markings, and props.
