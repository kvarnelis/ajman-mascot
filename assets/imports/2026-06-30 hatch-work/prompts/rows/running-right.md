Create one horizontal animation strip for Codex pet `ajman`, state `running-right`.
...
Identity: same pet in every frame: a black-and-white tuxedo cat matching the reference: black head, back, and sides; white chest bib, white front paws, one white front leg, yellow-green eyes, upright ears, slim seated cat silhouette, clean painterly sticker illustration. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `sticker`: Polished sticker mascot with bold clean shapes, crisp outline, flat colors, and minimal highlight detail. User style notes: clean painterly sticker illustration, crisp outline, expressive but restrained cat poses, no extra props.
