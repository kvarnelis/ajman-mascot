Create Codex pet row `running-right` for `ajman`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: a black-and-white tuxedo cat matching the reference: black head, back, and sides; white chest bib, white front paws, one white front leg, yellow-green eyes, upright ears, slim seated cat silhouette, clean painterly sticker illustration. Preserve silhouette, face, palette, material, proportions, markings, and props.
