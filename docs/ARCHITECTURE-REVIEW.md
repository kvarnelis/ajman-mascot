...
- Implicit bubble dismissal (Stage 4 path 3) is an **approximation**: we infer "user has seen the thread" from our own event stream (next `UserPromptSubmit` / attention-state exit) plus a stale-card timeout, because true "opened the thread in the app" is private to Codex/Claude and we take no Accessibility permission. Revisit if a reliable window/tab-focus signal becomes available cheaply.
