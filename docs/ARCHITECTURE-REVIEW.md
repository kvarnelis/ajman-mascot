...
- **Inter-cat glances — implemented 2026-07-13.** Cats stay mostly independent — the owner *likes* seeing Ajman stay calm while Winnie goes nuts, so interaction remains probabilistic rather than synchronized. `AppDelegate` owns a light coordinator notified when a shown pet begins `jumping`, `waving`, `running-right`, `running-left`, or `running`. Each other shown v2 pet is considered only while its live and displayed states are both idle; manual/Debug, sleep, an existing glance, Pet-Mode beats, and all agent-driven non-idle states are absolute exclusions. Eligible siblings independently react at 60%, with an eight-second per-pet cooldown. The reactor converts the vector between panel centers to the nearest of the 16 authored compass headings, holds that one look-row frame for 1.2–1.8 seconds, then restores idle and Pet-Mode timers. A bound-agent event, Debug action, click/wake, pet switch, or mode change cancels the glance immediately; v1 sheets skip it silently.

- **Per-cat control-click menu (owner idea 2026-07-13).** Control-click (or right-click) directly on a cat pops a context menu anchored **above that cat**, scoped to **that pet only** — pick a state/action (idle · running · waiting · review · failed · jumping · waving · sleep · wake · resume-live) and only that cat performs it. Removes the need to reach up to the status-bar Debug submenu, which today is **global** and forces *every* pet into manual mode. Side-benefit: makes the inter-cat glance directly testable — drive one cat lively while its sibling stays calm (the exact "one Debug pet, one calm pet" setup the global menu can't create). Impl sketch: `PetView` handles `rightMouseDown` / control+`leftMouseDown` → build an `NSMenu` and `popUp(positioning:at:in:)` above the panel; items call into that single `PetInstance`'s debug/sleep/wake/resume paths (per-pet, never global).
